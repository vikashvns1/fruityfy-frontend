...
const DealOfTheDay = () => {
    ...
    const bannerStyle = {
        backgroundImage: `linear-gradient(${isRTL ? 'to left' : 'to right'}, rgba(255, 248, 240, 1) 35%, rgba(255, 248, 240, 0) 100%), url(${getImageUrl(campaign.banner_image)})`,
        backgroundSize: 'cover',
    ...
    return (
        <section 
            ...
            className="rounded-3xl overflow-hidden shadow-lg border border-orange-100 my-6 relative max-w-[1536px] mx-auto min-h-[280px] md:min-h-[320px] flex items-center transition-all duration-300"
        >
            <div className={`relative z-10 p-6 md:p-12 w-full md:w-3/5 lg:w-1/2 ${isRTL ? 'text-right' : 'text-left'}`}>
                {/* Badge */}
                <div className="inline-flex items-center gap-2 bg-red-600 text-white px-3.5 py-1.5 rounded-full text-[10px] font-bold mb-4 tracking-wider uppercase shadow-md">
                    <MdLocalOffer size={14} /> {t('deal.badge')}
                </div>
                ...
                {/* Subtitle */}
                <div className={`text-gray-700 mb-6 text-sm md:text-lg font-semibold max-w-md ${isRTL ? 'mr-0' : ''}`}>
                   {isRTL ? (campaign.subtitle_ar || campaign.subtitle) : campaign.subtitle}
                </div>

                {/* Timer UI */}
                <div className={`flex gap-3 mb-8 ${isRTL ? 'flex-row-reverse justify-end' : ''}`}>
                    {[
                        { label: t('deal.days'), val: timeLeft.days },
                        { label: t('deal.hours'), val: timeLeft.hours },
                        { label: t('deal.mins'), val: timeLeft.minutes },
                        { label: t('deal.secs'), val: timeLeft.seconds }
                    ].map((item) => (
                        <div key={item.label} className="text-center">
                            <div className="w-12 h-12 md:w-16 md:h-16 bg-white/95 backdrop-blur-sm border-2 border-[#064E3B]/5 rounded-2xl flex items-center justify-center text-lg md:text-2xl font-black text-[#064E3B] shadow-lg">
                                {item.val.toString().padStart(2, '0')}
                ...
                >
                    {t('deal.grab_btn')} 
                    <MdArrowForward size={18} className={`${isRTL ? 'rotate-180' : 'group-hover:translate-x-2'} transition-transform`} />
                </button>
            </div>
        </section>
    );
