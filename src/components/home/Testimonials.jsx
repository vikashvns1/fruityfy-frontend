        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review) => (
            <div
              ...
              `}
            >
              {/* QUOTE ICON - Flipped for RTL */}
              <MdFormatQuote className={`absolute top-5 ${isRTL ? 'left-5 rotate-180' : 'right-5'} text-[#15803d]/15 text-5xl`} />

              {/* RATING */}
              <div className={`flex text-yellow-400 mb-4 ${isRTL ? 'justify-start flex-row-reverse' : ''}`}>
                {[...Array(5)].map((_, index) => (
                  <MdStar
                    key={index}
                    className={
              ...
              {/* REVIEW */}
              <p className="text-gray-600 text-sm leading-relaxed italic mb-8 min-h-[72px]">
                “{isRTL ? (review.review_text_ar || review.review_text) : review.review_text}”
              </p>
              ...
              <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={isRTL ? 'text-right' : 'text-left'}>
                  ...
                  <span className="text-xs text-green-600 font-medium">
                    {isRTL ? (review.designation_ar || t('testimonials.verified')) : (review.designation || t('testimonials.verified'))}
                  </span>
                </div>
              </div>
