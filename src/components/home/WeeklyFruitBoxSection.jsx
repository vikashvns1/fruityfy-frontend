...
const FruitBundleSection = () => {
  ...
  const handleAddToCart = (e, box) => {
    ...
    toast.success(`${qty} x ${isRTL ? (box.title_ar || box.title) : box.title} ${t('juice_builder.added_toast')}`, {
          ...
          {boxes.map(box => {
            ...
            return (
              <div 
                key={box.id} 
                onClick={() => navigate(`/weeklyboxes/${box.slug || box.id}`)}
                className={`group bg-white rounded-3xl shadow-soft hover:shadow-card transition-all duration-300 overflow-hidden relative border border-gray-100 flex flex-col cursor-pointer transform hover:-translate-y-1 ${isRTL ? 'text-right' : 'text-left'}`}
              >
                {/* Badges */}
                <div className={`absolute top-4 ${isRTL ? 'right-4' : 'left-4'} z-10 flex flex-col gap-2`}>
                    {savePercent > 0 && (
                    <span className="bg-red-500 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-md">
                        -{savePercent}%
                    </span>
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="mb-4">
                    ...
                    <p className="text-xs text-gray-400 mt-1 italic">
                        {isRTL ? (box.subtitle_ar || box.subtitle) : box.subtitle}
                    </p>
                    <p className="text-[10px] font-bold text-primaryLight uppercase mt-2">{t('bundle.freshness')}</p>
                  </div>

                  <div className={`mt-auto pt-4 border-t border-gray-50 flex items-end justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                    {/* Pricing */}
                    ...
                    <div className={`flex flex-col gap-2 ${isRTL ? 'items-start' : 'items-end'}`}>
                        <div className={`flex items-center gap-3 bg-gray-50 rounded-full p-1 border border-gray-100 ${isRTL ? 'flex-row-reverse' : ''}`}>
                            <button 
                                onClick={(e) => handleQtyChange(e, box.id, -1)}
                                className="w-7 h-7 rounded-full bg-white shadow-sm flex items-center justify-center text-primary hover:bg-primary hover:text-white transition"
                            >
                        ...
                        >
                            {isAdded ? (
                                <><Check size={16} /><span className="text-xs font-bold uppercase">{t('juice_builder.added')}</span></>
                            ) : (
                                <><Plus size={18} strokeWidth={3} /><span className="text-xs font-bold uppercase tracking-wider">{t('juice_builder.add')}</span></>
                            )}
                        </button>
                    </div>
                  </div>
        ...
        {boxes.length > 3 && (
            <div className="text-center mt-12">
                <button
                    ...
                    className="px-10 py-3 border-2 border-primary text-primary font-black rounded-full hover:bg-primary hover:text-white transition-all uppercase text-xs tracking-widest shadow-soft hover:shadow-lg"
                >
                    {t('bundle.view_all_bundles', { count: boxes.length })}
                </button>
            </div>
        )}
      </div>
