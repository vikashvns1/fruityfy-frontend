...
const ProductCard = ({ product, campaign = null, onWishlistChange }) => {
  ...
  const deliveryMap = {
    same_day: { label: t('product.today') || 'Today', icon: '⚡' },
    next_day: { label: t('product.tomorrow') || 'Tomorrow', icon: '📅' },
    two_to_four_days: { label: t('product.few_days') || '2–4 Days', icon: '🚚' },
  };
  ...
  return (
    ...
    >
      ...
      {/* ================= WISHLIST ================= */}
      <div className={`absolute top-3 ${isRTL ? 'left-3' : 'right-3'} z-20 opacity-0 group-hover:opacity-100 transition-opacity`}>
        <div className="bg-white rounded-full p-1.5 shadow-md hover:scale-110 transition">
          <WishlistHeart productId={product.id} onChange={onWishlistChange} />
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <p className="text-xs text-muted mb-4">
          {product.unit ? `${t('product.approx')} ${product.unit}` : t('product.per_item')}
        </p>
        ...
        <div className={`mt-auto flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          >
            {isAdded ? (
              <>
                <Check size={16} />
                <span className="hidden md:inline text-xs font-bold">{t('juice_builder.added')}</span>
              </>
            ) : (
              <>
                <Plus size={18} strokeWidth={3} />
