...
const ReviewModal = ({ productId, productName, onClose, onSuccess }) => {
    ...
    const handleImageChange = (e) => {
        const files = Array.from(e.target.files);
        if (files.length + images.length > 3) {
            toast.error(isRTL ? "يمكنك تحميل ما يصل إلى 3 صور فقط" : "You can only upload up to 3 images");
            return;
        }
        setImages([...images, ...files]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (res.success) {
            toast.success(isRTL ? "تم إرسال المراجعة!" : "Review submitted!");
            if (onSuccess) onSuccess(); 
            onClose();
        } else {
            toast.error(res.message);
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className={`bg-white rounded-[2rem] w-full max-w-md p-8 relative shadow-2xl animate-in zoom-in-95 duration-300 ${isRTL ? 'text-right' : 'text-left'}`}>
                
                {/* Close Button */}
                <button 
                    onClick={onClose} 
                    className={`absolute top-6 ${isRTL ? 'left-6' : 'right-6'} p-2 bg-gray-50 rounded-full text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all`}
                >
                    <X size={20} />
                </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        <textarea 
                            required 
                            rows="4" 
                            placeholder={isRTL ? "كيف كان المنتج؟ أخبرنا عن الجودة والطزاجة..." : "How was the product? Tell us about the freshness and quality..."}
                            value={comment} 
                            onChange={(e) => setComment(e.target.value)} 
                            className={`w-full p-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-green-500 text-sm font-medium placeholder:text-gray-400 resize-none transition-all ${isRTL ? 'text-right' : ''}`}
                        ></textarea>
                    </div>

                    {/* Image Upload */}
                    <div className="space-y-3">
                        <p className={`text-[11px] font-black text-gray-400 uppercase tracking-widest ${isRTL ? 'pr-1' : 'pl-1'}`}>
                            {isRTL ? 'الصور (اختياري)' : 'Photos (Optional)'}
                        </p>
                        <div className={`flex flex-wrap gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                            <label className="w-16 h-16 flex flex-col items-center justify-center gap-1 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:bg-green-50 hover:border-green-300 transition-all text-gray-400 hover:text-green-600">
                                <Camera size={20} />
                                <span className="text-[9px] font-bold">{isRTL ? 'إضافة' : 'Add'}</span>
                                <input type="file" multiple accept="image/*" className="hidden" onChange={handleImageChange} />
                            </label>
                    ...
                    <button 
                        ...
                        className="w-full bg-green-600 text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg shadow-green-100 hover:bg-green-700 hover:shadow-green-200 active:scale-[0.98] transition-all disabled:bg-gray-200 disabled:shadow-none"
                    >
                        {loading ? (isRTL ? 'جاري النشر...' : "Posting Review...") : (isRTL ? 'إرسال التقييم' : "Submit My Review")}
                    </button>
                </form>
            </div>
        </div>
