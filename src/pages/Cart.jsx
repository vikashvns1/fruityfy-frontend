...
import { useSettings } from '../context/SettingsContext';
import { useAuth } from '../context/AuthContext';
import { validateCouponApi, getImageUrl } from '../services/api';
...
const Cart = () => {
    ...
    const handleApplyCoupon = async () => {
        if (!couponCode.trim()) {
            toast.error(isRTL ? "يرجى إدخال الرمز" : "Please enter a code");
            return;
        }
        setCouponLoading(true);
        ...
        if (res.success) {
            setDiscount(res.data.discountAmount);
            setAppliedCoupon(res.data.code);
            toast.success(isRTL ? `تم تطبيق الكوبون ${res.data.code}` : `Coupon ${res.data.code} Applied!`);
        } else {
            toast.error(res.message);
            setDiscount(0);
            setAppliedCoupon(null);
    ...
    const handleCheckout = () => {
        if (user) {
            ...
            toast.error(t('auth.login_required') || "Please login to complete your order");
            navigate('/login', { state: { from: '/cart' } });
        }
    };
    ...
    if (cartItems.length === 0) {
        return (
            <div className="min-h-[80vh] flex flex-col items-center justify-center bg-[#f1f3f6] text-center p-4">
                <img src="https://cdn-icons-png.flaticon.com/512/11329/11329060.png" alt="Empty Cart" className="w-48 mb-6 opacity-80 mix-blend-multiply" />
                <h2 className="text-2xl font-bold text-gray-800 mb-2">{t('cart.empty_msg')}</h2>
                <button onClick={() => navigate('/shop')} className="bg-[#15803d] text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-green-800 transition-all flex items-center gap-2">
                    {t('common.continue_shopping')} {isRTL ? <ArrowLeft size={18} /> : <ArrowRight size={18} />}
                </button>
            </div>
        );
    }
    ...
    return (
        <div className={`bg-[#f1f3f6] min-h-screen py-8 font-sans ${isRTL ? 'text-right' : 'text-left'}`}>
            <div className="w-[95%] max-w-[1280px] mx-auto">
                <h1 className={`text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <ShoppingBag className="text-[#15803d]" /> {t('cart.title')} ({cartItems.length})
                </h1>
                ...
                <div className={`flex flex-col lg:flex-row gap-8 ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
                    ...
                    <div className="flex-1 space-y-4">
                        {cartItems.map((item) => (
                            <div key={item.id} className={`bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex gap-4 items-center ${isRTL ? 'flex-row-reverse' : ''}`}>

                                {/* Image Section */}
                                <div className="w-20 h-20 bg-gray-50 rounded-md border border-gray-200 p-2 flex-shrink-0 relative">
                                    <img
                                        src={getImageUrl(item.media_url || item.image_url || item.image, "https://cdn-icons-png.flaticon.com/512/2748/2748558.png")}
                                        alt={isRTL ? (item.name_ar || item.name) : item.name}
                                        className="w-full h-full object-contain mix-blend-multiply"
                                    />
                                    {item.is_custom && (
                                        <div className={`absolute -top-2 ${isRTL ? '-right-2' : '-left-2'} bg-[#064e3b] text-[#eab308] p-1 rounded-full shadow-md`}>
                                            <MdOutlineScience size={12} />
                                        </div>
                                    )}
                                </div>
                                    ...
                                    {item.is_custom && item.configuration ? (
                                        ...
                                        <div className="mt-1 bg-orange-50/50 p-2 rounded border border-orange-100/50">
                                            <p className={`text-[9px] font-black text-[#854d0e] uppercase tracking-widest mb-1 flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                📦 {isRTL ? 'محتويات السلة:' : 'Basket Contents:'}
                                            </p>
                                            <div className={`flex flex-wrap gap-x-3 gap-y-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                {item.bundle_items && item.bundle_items.length > 0 ? (
                                                    item.bundle_items.map((prod, i) => (
                                                    ...
                                                    <span className="text-[10px] text-gray-400 italic">{isRTL ? 'لا توجد عناصر' : 'No items found'}</span>
                                                )}
                                            </div>
                                        </div>
                                    ) : (
                                ...
                                <div className={`flex flex-col items-end gap-3 ${isRTL ? 'items-start' : 'items-end'}`}>
                                    <div className={`flex items-center border border-gray-300 rounded-md bg-gray-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 hover:bg-gray-200 text-gray-600"><Minus size={14} /></button>
                                        <span className="w-8 text-center text-sm font-bold">{item.quantity}</span>
                                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 hover:bg-gray-200 text-gray-600"><Plus size={14} /></button>
                                    </div>
                                    <button onClick={() => removeFromCart(item.id)} className={`text-xs font-bold text-red-500 hover:text-red-700 flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className="lg:w-[350px] space-y-4">
                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 sticky top-24">
                            <h3 className="font-bold text-gray-800 text-lg mb-4 border-b border-gray-100 pb-2">{t('cart.bill_details')}</h3>
                            ...
                            <div className="space-y-3 text-sm text-gray-600 mb-4">
                                ...
                                <div className={`flex justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                                    <span className={`flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                        {t('cart.vat')} ({settings.tax_rate}%) <Info size={12} className="text-gray-400" />
                                    </span>
                                    <span className="font-medium">{formatPrice(taxAmount)}</span>
                                </div>
                                ...
                                <div className="pt-2 pb-2">
                                    {appliedCoupon ? (
                                        <div className={`flex justify-between items-center bg-green-50 border border-green-200 p-2 rounded-md ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                <Tag size={14} className="text-green-600" />
                                                <span className="font-bold text-green-700 text-xs">{appliedCoupon} {t('cart.coupon_applied')}</span>
                                            </div>
                                            <button onClick={handleRemoveCoupon} className="text-gray-400 hover:text-red-500"><X size={16} /></button>
                                        </div>
                                    ) : (
                                        <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <input
                                                ...
                                                className={`flex-1 border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-green-600 uppercase ${isRTL ? 'text-right' : 'text-left'}`}
                                                value={couponCode}
                                                onChange={(e) => setCouponCode(e.target.value.toUpperCase())}
                                            />
                                            <button
                                ...
                                {discount > 0 && (
                                    <div className={`flex justify-between text-green-600 font-bold border-t border-dashed border-gray-200 pt-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                ...
                                {deliveryFee > 0 && (
                                    <div className="mt-2">
                                        <div className={`flex justify-between text-[10px] text-gray-500 mb-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span>{isRTL ? `أضف ${formatPrice(settings.free_delivery_threshold - subTotal)} للتوصيل المجاني` : `Add ${formatPrice(settings.free_delivery_threshold - subTotal)} for Free Delivery`}</span>
                                            <span>{Math.round((subTotal / settings.free_delivery_threshold) * 100)}%</span>
                                        </div>
                                        <div className="w-full bg-gray-200 rounded-full h-1.5">
                                            <div className={`bg-[#eab308] h-1.5 rounded-full transition-all duration-500 ${isRTL ? 'float-right' : ''}`} style={{ width: `${Math.min((subTotal / settings.free_delivery_threshold) * 100, 100)}%` }}></div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {deliveryFee === 0 && (
                                    <div className="text-[10px] text-green-700 bg-green-50 p-2 rounded text-center font-bold border border-green-100">
                                        {t('cart.unlocked_free')}
                                    </div>
                                )}
                            </div>

                            <div className={`flex justify-between font-bold text-gray-900 text-lg border-t border-dashed border-gray-300 pt-4 mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                <span>{t('cart.total_pay')}</span>
                                <span>{formatPrice(finalTotal)}</span>
                            </div>
                            ...
                            <div className={`mt-4 flex items-center justify-center gap-2 text-[10px] text-gray-400 ${isRTL ? 'flex-row-reverse' : ''}`}>
