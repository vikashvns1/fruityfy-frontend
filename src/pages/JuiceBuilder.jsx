...
import {
    MdLocalDrink, MdAddShoppingCart, MdBolt, MdCheckCircle,
    MdSettings, MdAutoFixHigh, MdOutlineScience, MdArrowForward, MdArrowBack,
    MdTune, MdFastfood, MdInfoOutline, MdAdd, MdAccessTime, MdClose
} from 'react-icons/md';
import Swal from 'sweetalert2';
...
const JuiceBuilder = () => {
    ...
    useEffect(() => {
        const fetchEverything = async () => {
            try {
                ...
                console.error("Initialization Error:", err);
                toast.error(isRTL ? "فشل تحميل بيانات المختبر" : "Failed to load lab data");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleAddToCart = (juice) => {
        addToCart(juice, 1);
        toast.success(isRTL ? `${juice.name_ar || juice.name} تمت إضافته إلى السلة!` : `${juice.name} added to cart!`);
    };
    ...
    const handleCheckoutCustom = () => {
        ...
        addToCart(customJuice, 1);
        toast.success(isRTL ? "تمت إضافة المزيج إلى السلة!" : "Custom masterpiece added to cart!");
        navigate('/cart');
    };
    ...
    const handleRemoveMasterpiece = (e) => {
        e.stopPropagation();
        Swal.fire({
            title: isRTL ? 'إزالة التحفة؟' : 'Remove Masterpiece?',
            text: isRTL ? "هل أنت متأكد من حذف الوصفة المحفوظة؟" : "Are you sure you want to remove your saved masterpiece?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#064E3B',
            cancelButtonColor: '#d33',
            confirmButtonText: isRTL ? 'نعم، احذفها' : 'Yes, remove it!',
            background: '#F8FAFA',
            borderRadius: '2rem',
        }).then((result) => {
            if (result.isConfirmed) {
    ...
    if (loading) return (
        <div className="min-h-screen bg-[#064E3B] flex flex-col items-center justify-center text-[#FACC15]">
            ...
            <p className="font-serif italic tracking-widest animate-pulse">{t('common.loading')}</p>
        </div>
    );

    return (
        <div className={`min-h-screen bg-[#F8FAFA] font-sans text-[#1A2E28] ${isRTL ? 'text-right' : 'text-left'}`}>
            <div className={`${!data ? "bg-[#061916] py-20" : "bg-white pt-24"} transition-all duration-700 relative overflow-hidden`}>
                <div className="max-w-[1440px] mx-auto px-6">
                    {!data ? (
                        <div className="animate-in fade-in duration-1000">
                            <div className="text-center mb-16">
                                <div className="inline-flex items-center gap-2 bg-[#FACC15] text-[#064E3B] px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-widest mb-6">
                                    <MdOutlineScience /> {isRTL ? 'مختبرنا الخاص' : 'Signature Lab'}
                                </div>
                                <h2 className="text-6xl md:text-8xl font-serif font-bold text-white mb-6">
                                    {isRTL ? 'مختبر' : 'The Juice'} <span className="text-[#FACC15] italic">{isRTL ? 'العصير' : 'Lab'}</span>
                                </h2>
                                <p className="text-green-100/50 max-w-2xl mx-auto text-lg leading-relaxed">
                                    {isRTL ? 'اختر تركيبة أساسية تم تصميمها من قبل خبراء التغذية - ثم قم بتخصيص كل ملليلتر.' : 'Choose a base formula engineered by nutrition experts — then fine-tune every milliliter.'}
                                </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full mt-12">
                                {templates.filter(t => t.id !== 81).map((t, idx) => (
                                    <div
                                        ...
                                        className="group relative bg-white/[0.06] hover:bg-white/[0.1] border border-white/10 rounded-[2.5rem] p-8 cursor-pointer transition-all duration-500 hover:shadow-2xl flex flex-col h-full"
                                    >
                                        <div className={`absolute top-5 ${isRTL ? 'right-6' : 'left-6'} text-white/5 font-serif text-6xl italic group-hover:text-[#FACC15]/10 transition-colors`}>
                                            {String(idx + 1).padStart(2, "0")}
                                        </div>
                                        <div className="relative z-10 w-full h-48 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-700">
                                            <img src={getImageUrl(t.image_url)} alt={t.name} className="h-full object-contain drop-shadow-2xl" />
                                        ...
                                        <div className="flex-1 space-y-3">
                                            <h3 className="text-xl font-bold text-white group-hover:text-[#FACC15] transition">{isRTL ? (t.name_ar || t.name) : t.name}</h3>
                                            <p className="text-white/40 text-xs leading-relaxed line-clamp-3 italic">
                                                {isRTL ? (t.full_description_ar || t.short_description_ar || t.full_description) : (t.full_description || t.short_description)}
                                            </p>
                                        </div>
                                    </div>
                                ))}
                                        ...
                                        return (
                                            <div onClick={() => navigate(`/juice-builder/custom`, { state: { initialSelections: savedRecipe.ingredients, recipeName: savedRecipe.name } })}
                                                ...
                                                <div className="flex-1 space-y-2">
                                                    ...
                                                    <button className="w-full py-4 bg-[#FACC15] text-[#064E3B] rounded-2xl font-black text-[10px] uppercase tracking-widest group-hover:bg-white transition-all shadow-xl">{isRTL ? 'إعادة مزج ابتكارك' : 'REMIX YOUR CREATION'}</button>
                                                </div>
                                            </div>
                                        );
                                    } else {
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in slide-in-from-bottom-8 duration-700">
                            <div className="lg:col-span-8 space-y-8">
                                <div className={`flex justify-between items-end border-b border-gray-100 pb-8 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                    <div>
                                        <button onClick={handleBackToLab} className={`text-[#064E3B] font-black text-[10px] uppercase tracking-widest mb-4 flex items-center gap-2 group ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span className={`${isRTL ? 'group-hover:translate-x-1' : 'group-hover:-translate-x-1'} transition-transform`}>{isRTL ? '→' : '←'}</span> {isRTL ? 'خروج' : 'Exit Lab'}
                                        </button>
                                ...
                                <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${isRTL ? 'direction-rtl' : ''}`}>
                                    <div className="relative overflow-hidden bg-green-50 p-6 rounded-[2rem] border border-green-100 group transition-all">
                                        <div className={`absolute ${isRTL ? '-left-4' : '-right-4'} -bottom-4 opacity-10 group-hover:scale-110 transition-transform duration-500`}><MdOutlineScience size={80} /></div>
                                        <div className="relative z-10">
                                            <h5 className={`text-[10px] font-black text-green-800 uppercase tracking-widest mb-2 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}><MdOutlineScience size={16} /> {isRTL ? 'الملف الغذائي' : 'Nutritional Profile'}</h5>
                                            <p className="text-xs text-green-700/80 leading-relaxed italic font-medium">{isRTL ? (data.product.nutritional_benefits_ar || 'تم تحسينه بناءً على المكونات.') : data.product.nutritional_benefits}</p>
                                        </div>
                                    </div>
                                    <div className="relative overflow-hidden bg-blue-50 p-6 rounded-[2rem] border border-blue-100 group transition-all">
                                        <div className={`absolute ${isRTL ? '-left-4' : '-right-4'} -bottom-4 opacity-10 group-hover:scale-110 transition-transform duration-500`}><MdAccessTime size={80} /></div>
                                        <div className="relative z-10">
                                            <h5 className={`text-[10px] font-black text-blue-800 uppercase tracking-widest mb-2 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}><MdAccessTime size={14} /> {isRTL ? 'العناية والتخزين' : 'Care & Storage'}</h5>
                                            <p className="text-xs text-blue-700/80 leading-relaxed font-medium">{isRTL ? (data.product.shelf_life_ar || 'يستهلك طازجاً خلال 24 ساعة.') : data.product.shelf_life}</p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="bg-white/5 rounded-3xl p-6 border border-white/5">
                                            <div className={`flex justify-between items-center mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                ...
                                                <span className="text-lg font-serif">{totals.calories} <span className="text-[10px]">{isRTL ? 'سعرة' : 'kcal'}</span></span>
                                            </div>
                                            <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                                                <div className={`h-full bg-[#FACC15] transition-all duration-500 ${isRTL ? 'float-right' : ''}`} style={{ width: `${Math.min((totals.calories / 600) * 100, 100)}%` }}></div>
                                            </div>
                                        </div>
                                        <div className={`flex justify-between items-baseline px-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span className="text-2xl font-serif">{isRTL ? 'الإجمالي' : 'Total'}</span>
                                            <div className={isRTL ? 'text-left' : 'text-right'}>
                                                <div className="text-4xl font-serif text-[#FACC15]">{formatPrice(totals.price)}</div>
                                                <div className="text-[10px] uppercase font-bold opacity-40">{isRTL ? 'شامل الضريبة والتغليف' : 'Inc. GST & Packing'}</div>
                                            </div>
                                        </div>
            ...
            <div className="bg-white py-24">
                <div className="max-w-[1440px] mx-auto px-6 relative z-10">
                    <div className={`flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
                        <div className={isRTL ? 'text-right' : ''}>
                            <h3 className="text-4xl font-serif font-bold text-[#064E3B]">{isRTL ? 'كلاسيكيات طازجة' : 'Freshly Squeezed Classics'}</h3>
                            <p className="text-gray-400 mt-2">{isRTL ? 'بدون تخصيص، كمال تام.' : 'Zero customization, pure perfection.'}</p>
                        </div>
                        <div className="h-px flex-1 bg-gray-100 mx-10 hidden md:block"></div>
                        <button onClick={() => navigate('/shop')} className="text-[#064E3B] font-bold text-xs uppercase tracking-widest border-2 border-[#064E3B]/10 px-6 py-3 rounded-xl hover:bg-[#064E3B] hover:text-white transition-all">{isRTL ? 'عرض الكل' : 'View All Menu'}</button>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-8">
                        {simpleJuices.map(juice => (
                            <div key={juice.id} className={`group bg-[#F8FAFA] rounded-[3rem] p-6 hover:bg-white hover:shadow-2xl transition-all duration-500 border border-transparent hover:border-gray-100 ${isRTL ? 'text-right' : ''}`}>
                                <div className="relative overflow-hidden rounded-[2rem] mb-6 bg-white aspect-square flex items-center justify-center p-8">
                                    <img src={getImageUrl(juice.image_url)} className="w-full h-full object-contain group-hover:scale-110 transition duration-700" alt={juice.name} />
                                    <div className={`absolute top-4 ${isRTL ? 'left-4' : 'right-4'} bg-white/80 backdrop-blur-md px-3 py-1 rounded-full text-[9px] font-black text-[#064E3B] uppercase tracking-tighter`}>{isRTL ? 'الأكثر مبيعاً' : 'Best Seller'}</div>
                                </div>
                                <h4 className="font-bold text-[#064E3B] text-xl mb-1">{isRTL ? (juice.name_ar || juice.name) : juice.name}</h4>
                                <p className="text-[11px] text-gray-400 font-medium mb-6 line-clamp-1">{isRTL ? (juice.short_description_ar || juice.short_description) : (juice.short_description || "Traditional cold-press method.")}</p>
                                <div className={`flex justify-between items-center mt-auto ${isRTL ? 'flex-row-reverse' : ''}`}>
                                    <span className="text-2xl font-serif font-bold text-[#064E3B]">{formatPrice(juice.price)}</span>
                                    <button onClick={() => handleAddToCart(juice)} className="bg-[#064E3B] text-white p-4 rounded-2xl hover:bg-[#FACC15] hover:text-[#064E3B] transition-all shadow-xl active:scale-90"><MdAddShoppingCart size={20} /></button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="py-12 border-t border-gray-100 text-center">
                <div className={`flex justify-center gap-8 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase text-gray-400"><MdFastfood className="text-[#FACC15]" /> {isRTL ? 'عضوي 100%' : '100% Organic'}</div>
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase text-gray-400"><MdBolt className="text-[#FACC15]" /> {isRTL ? 'معصور على البارد' : 'Cold Pressed'}</div>
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase text-gray-400"><MdCheckCircle className="text-[#FACC15]" /> {isRTL ? 'تم اختباره' : 'Lab Tested'}</div>
                </div>
            </div>
        </div>
    );
