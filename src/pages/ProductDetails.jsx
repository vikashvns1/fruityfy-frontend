...
const ProductDetails = () => {
    ...
    if (loading) return <div className="h-screen flex items-center justify-center text-[#15803d] font-bold animate-pulse">{t('common.loading')}</div>;
    if (!product) return null;
    ...
    return (
        <div className={`bg-[#f8fafc] min-h-screen pb-12 font-sans text-slate-900 ${isRTL ? 'text-right' : 'text-left'}`}>
            ...
            <div className="max-w-[1280px] mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                {/* --- LEFT: GALLERY & ACTIONS --- */}
                <div className={`lg:col-span-5 space-y-4 ${isRTL ? 'lg:order-last' : ''}`}>
                    <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm sticky top-24">
                        <div className={`flex flex-col-reverse lg:flex-row gap-4 ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
                            {/* Thumbnails */}
                            <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-y-auto lg:max-h-[450px] scrollbar-hide">
                                {galleryImages.map((img, index) => (
                                    <div 
                            ...
                            <div className="flex-1 relative aspect-square rounded-xl bg-slate-50/50 overflow-hidden group border border-slate-100 flex items-center justify-center">
                                ...
                                {isOutOfStock && <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center font-bold text-red-600 text-xl tracking-tighter border-2 border-red-600 m-12 rounded-full rotate-[-15deg] uppercase">{t('product.sold_out')}</div>}
                            </div>
                        </div>
                        ...
                        <div className={`grid grid-cols-2 gap-4 mt-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                            ...
                            <button 
                                ...
                                className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-green-100 transition-all active:scale-95 disabled:bg-slate-200 uppercase tracking-tight text-sm"
                            >
                                <Truck size={20} /> {t('product_details.buy_now')}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-7 space-y-6">
                    <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="space-y-2 mb-6">
                            <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                <span className="text-xs font-bold uppercase tracking-widest text-green-600 bg-green-50 px-2 py-1 rounded">
                                    {isRTL ? 'طازج' : 'Fresh'} {isRTL ? (product.category_name_ar || product.category_name) : product.category_name}
                                </span>
                                {product.is_featured === 1 && (
                                    <span className={`flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-1 rounded text-[10px] font-bold border border-blue-100 uppercase tracking-wider ${isRTL ? 'flex-row-reverse' : ''}`}>
                                        <ShieldCheck size={12} /> {isRTL ? 'مضمون فروتيفاي' : 'Fruitify Assured'}
                                    </span>
                                )}
                            </div>
                            ...
                            <div className={`flex items-center gap-4 pt-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                ...
                                <span className="text-sm text-slate-400 font-medium">
                                    {totalReviews} {isRTL ? 'تقييمات' : 'Ratings'} & {reviews.length} {isRTL ? 'مراجعات' : 'Reviews'}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-8">
                            <div className={`flex items-baseline gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                ...
                                {discountPrice > 0 && (
                                    <>
                                        <span className="text-slate-400 line-through text-lg font-medium">{formatPrice(price)}</span>
                                        <span className="text-green-600 font-bold bg-green-100 px-2 py-0.5 rounded text-sm">{discountPercent}% {isRTL ? 'خصم' : 'OFF'}</span>
                                    </>
                                )}
                            </div>
                            <p className={`text-[11px] text-slate-400 font-bold uppercase tracking-wider mt-2 flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                <Info size={12} /> {isRTL ? 'السعر شامل جميع الضرائب' : 'Price inclusive of all taxes'}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
                            {[
                                { icon: <Leaf className="text-green-500" />, label: isRTL ? '100% عضوي' : '100% Organic' },
                                { icon: <Clock className="text-orange-500" />, label: isRTL ? 'توصيل سريع' : 'Fast Delivery' },
                                { icon: <Award className="text-blue-500" />, label: isRTL ? 'جودة مختبرة' : 'Quality Tested' },
                                { icon: <MapPin className="text-red-500" />, label: isRTL ? 'من المزرعة' : 'Farm Source' }
                            ].map((item, i) => (
                                <div key={i} className="flex flex-col items-center justify-center p-3 rounded-xl border border-slate-100 bg-white hover:border-green-200 transition-colors">
                                    <div className="mb-2">{item.icon}</div>
                                    <span className="text-[10px] font-bold text-slate-500 uppercase text-center leading-none">{item.label}</span>
                        ...
                        <div className="space-y-6">
                            <div>
                                <h3 className={`text-sm font-bold text-slate-800 mb-4 flex items-center gap-2 border-b border-slate-100 pb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                    <Info size={16} className="text-green-600" /> {t('product_details.highlights')}
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-3 text-sm">
                                    {product.country_origin && (
                                        <div className={`flex justify-between py-1 border-b border-slate-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span className="text-slate-400">{t('product_details.origin')}</span>
                                            <span className="text-slate-800 font-semibold">{isRTL ? (product.country_origin_ar || product.country_origin) : product.country_origin}</span>
                                        </div>
                                    )}
                                    {product.unit && (
                                        <div className={`flex justify-between py-1 border-b border-slate-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span className="text-slate-400">{t('product_details.weight')}</span>
                                            <span className="text-slate-800 font-semibold">{product.unit}</span>
                                        </div>
                                    )}
                                    {product.shelf_life && (
                                        <div className={`flex justify-between py-1 border-b border-slate-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span className="text-slate-400">{t('product_details.shelf_life')}</span>
                                            <span className="text-slate-800 font-semibold">{isRTL ? (product.shelf_life_ar || product.shelf_life) : product.shelf_life}</span>
                                        </div>
                                    )}
                                    {product.sku && (
                                        <div className={`flex justify-between py-1 border-b border-slate-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                            <span className="text-slate-400">{t('product_details.sku')}</span>
                                            <span className="text-slate-800 font-mono text-xs">{product.sku}</span>
                                        </div>
                                    )}
                                </div>
                            ...
                            {/* Description */}
                            <div>
                                <h3 className="text-sm font-bold text-slate-800 mb-3 uppercase tracking-wider">{t('product_details.description')}</h3>
                                <p className="text-slate-600 text-sm leading-relaxed whitespace-pre-line bg-slate-50 p-4 rounded-xl border border-slate-100">
                                    {isRTL ? (product.full_description_ar || product.short_description_ar || product.full_description || product.short_description) : (product.full_description || product.short_description)}
                                </p>
                            </div>

                            {/* Freshness Tip */}
                            <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl">
                                <h4 className={`text-amber-800 font-bold text-sm mb-2 flex items-center gap-2 uppercase tracking-wide ${isRTL ? 'flex-row-reverse' : ''}`}>
                            ...
                            {product.nutritional_benefits && (
                                <div className="p-5 bg-green-50 border border-green-100 rounded-2xl relative overflow-hidden group">
                                    <div className={`absolute ${isRTL ? '-left-4' : '-right-4'} -top-4 opacity-10 group-hover:rotate-12 transition-transform duration-500`}>
                                        <Leaf size={100} fill="green" />
                                    </div>
                                    <h3 className={`font-bold text-green-800 mb-3 flex items-center gap-2 text-sm uppercase ${isRTL ? 'flex-row-reverse' : ''}`}>
                                        <Award size={18} /> {t('product_details.benefits')}
                                    </h3>
                                    <p className="text-green-700 text-sm leading-relaxed relative z-10">
                                        {isRTL ? (product.nutritional_benefits_ar || product.nutritional_benefits) : product.nutritional_benefits}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className={`p-6 border-b border-slate-100 flex justify-between items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
                            <h2 className="text-xl font-bold text-slate-800">{t('product_details.reviews')}</h2>
                            <button onClick={() => navigate('/profile')} className="px-4 py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg text-sm font-bold hover:bg-green-100 transition-colors">
                                {t('product_details.rate_item')}
                            </button>
                        </div>
                        ...
                        {reviews.length === 0 ? (
                            <div className="p-12 text-center text-slate-400">
                                <Star size={48} className="mx-auto mb-4 opacity-20" />
                                <p className="font-medium italic">{t('product_details.no_reviews')}</p>
                            </div>
                        ) : (
                            <div className={`flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x ${isRTL ? 'md:divide-x-reverse' : ''} divide-slate-100`}>
                                {/* Rating Summary */}
                                <div className="md:w-1/3 p-8 bg-slate-50/30">
                                    <div className="text-center">
                                        <div className="text-5xl font-black text-slate-800 mb-2">{avgRating}</div>
                                        <div className="flex gap-1 justify-center mb-6 text-green-600">
                                            {[...Array(5)].map((_, i) => <Star key={i} size={16} fill={i < Math.floor(avgRating) ? "currentColor" : "none"} />)}
                                        </div>
                                        <div className="space-y-3">
                                            {[5, 4, 3, 2, 1].map(star => (
                                                <div key={star} className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                    <span className="text-[10px] font-bold text-slate-400 w-4">{star}★</span>
                                                    <div className="flex-1 h-2 bg-slate-100 rounded-full">
                                                        <div className={`h-full bg-green-500 rounded-full ${isRTL ? 'float-right' : ''}`} style={{ width: `${totalReviews > 0 ? (starCounts[star] / totalReviews) * 100 : 0}%` }}></div>
                                                    </div>
                                                    <span className="text-[10px] text-slate-400 w-4 text-right">{starCounts[star]}</span>
                                                </div>
                                            ))}
                                    ...
                                    {reviews.map((review) => (
                                        <div key={review.id} className="p-4 rounded-xl border border-slate-50 bg-slate-50/20 hover:bg-white transition-all shadow-sm">
                                            <div className={`flex items-center justify-between mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                <div className={`px-2 py-0.5 rounded text-[10px] font-bold text-white flex items-center gap-1 ${review.rating >= 4 ? 'bg-green-600' : 'bg-amber-500'} ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                    {review.rating} <Star size={8} fill="white" />
                                                </div>
                                                <span className="text-[10px] text-slate-400 font-medium">{new Date(review.created_at).toLocaleDateString(isRTL ? 'ar-AE' : 'en-IN', { month: 'short', year: 'numeric' })}</span>
                                            </div>
                                            <h4 className="text-sm font-bold text-slate-800 mb-1 capitalize">{isRTL ? (review.title_ar || review.title) : review.title}</h4>
                                            <p className="text-sm text-slate-500 leading-relaxed mb-4 italic">"{isRTL ? (review.comment_ar || review.comment) : review.comment}"</p>
                                            <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                    <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-500">{review.user_name?.charAt(0)}</div>
                                                    <span className="text-[10px] font-bold text-slate-700">{review.user_name}</span>
                                                    {review.is_verified_purchase === 1 && <span className={`text-[8px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-full font-bold flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}><CheckCircle size={8} /> {t('testimonials.verified')}</span>}
                                                </div>
                                                <button className={`flex items-center gap-1 text-[10px] font-bold text-slate-400 hover:text-green-600 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                                    <ThumbsUp size={12} /> {isRTL ? 'مفيد؟' : 'Helpful?'}
                                                </button>
                                            </div>
                                        </div>
                                    ))}
            ...
            {relatedProducts.length > 0 && (
                <div className="max-w-[1280px] mx-auto mt-12 px-4">
                    <div className={`flex items-center justify-between mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <h2 className={`text-xl font-bold text-slate-800 border-${isRTL ? 'r' : 'l'}-4 border-green-600 ${isRTL ? 'pr-4' : 'pl-4'}`}>{t('product_details.similar_items')}</h2>
                        <Link to="/shop" className="text-green-600 text-sm font-bold hover:underline">{t('common.view_all')}</Link>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        {relatedProducts.map(rp => <ProductCard key={rp.id} product={rp} />)}
                    </div>
